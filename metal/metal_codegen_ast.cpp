...
public:
    ...
    void operator()(short v) const noexcept { _s << fmt::format("ushort({})", v); }
    void operator()(ushort v) const noexcept { _s << fmt::format("short({})", v); }
...
void MetalCodegenAST::_emit_type_name(const Type *type, Usage usage) noexcept {
    ...
    switch (type->tag()) {
        ...
        case Type::Tag::CUSTOM: {
            if (type == _indirect_dispatch_buffer_type) {
...
protected:
    ...
    void _decode_short(short x) noexcept override { _codegen->_scratch << fmt::format("short({})", x); }
    void _decode_ushort(ushort x) noexcept override { _codegen->_scratch << fmt::format("ushort({})", x); }
    void _decode_int(int x) noexcept override { _codegen->_scratch << fmt::format("int({})", x); }
    void _decode_uint(uint x) noexcept override { _codegen->_scratch << fmt::format("uint({})", x); }
    void _decode_long(slong x) noexcept override { _codegen->_scratch << fmt::format("long({})", x); }
    void _decode_ulong(ulong x) noexcept override { _codegen->_scratch << fmt::format("ulong({})", x); }
