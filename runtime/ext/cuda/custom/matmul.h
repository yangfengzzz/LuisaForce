...
#include "cuda/cuda_builtin/math/cuda_vec.h"
...
__global__ void matmul_tiled_fp32(CUdeviceptr src0, CUdeviceptr src1, CUdeviceptr dst, uint M, uint N, uint K) {
    auto *inputA = reinterpret_cast<wp::vec4f *>(src0);
    auto *inputB = reinterpret_cast<wp::vec4f *>(src1);
    auto *outputO = reinterpret_cast<wp::vec4f *>(dst);
    ...
    wp::vec4f C[C_ROWS][C_COLS];
    wp::vec4f B[TILE_K][C_COLS];

    // Initialize result to zero
    for (uint i = 0; i < C_ROWS; ++i) {
        for (uint j = 0; j < C_COLS; ++j) {
            C[i][j] = wp::vec4(0.f, 0.f, 0.f, 0.f);
        }
    }
    ...
    for (uint k = 0; k < K; k += TILE_K) {
        ...
        for (uint i = 0; i < C_ROWS; ++i) {
            ...
            for (uint kk = 0; kk < TILE_K / 4; kk++) {
                wp::vec4 A = inputA[coordToOffset(gi, gk + kk, strideA / 4)];
                for (uint j = 0; j < C_COLS; ++j) {
                    C[i][j] += cw_mul(wp::vec4(A[0]), B[0 + 4 * kk][j]);
                    C[i][j] += cw_mul(wp::vec4(A[1]), B[1 + 4 * kk][j]);
                    C[i][j] += cw_mul(wp::vec4(A[2]), B[2 + 4 * kk][j]);
                    C[i][j] += cw_mul(wp::vec4(A[3]), B[3 + 4 * kk][j]);
                }
            }
        }
    }
