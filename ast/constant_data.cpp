...
protected:
    void _decode_bool(bool x) noexcept override { _s.append(fmt::format("bool({})", x)); }
    void _decode_short(short x) noexcept override { _s.append(fmt::format("short({})", x)); }
    void _decode_ushort(ushort x) noexcept override { _s.append(fmt::format("ushort({})", x)); }
    void _decode_int(int x) noexcept override { _s.append(fmt::format("int({})", x)); }
    void _decode_uint(uint x) noexcept override { _s.append(fmt::format("uint({})", x)); }
    void _decode_long(slong x) noexcept override { _s.append(fmt::format("long({})", x)); }
    void _decode_ulong(ulong x) noexcept override { _s.append(fmt::format("ulong({})", x)); }
    void _decode_half(half x) noexcept override { _s.append(fmt::format("half({})", static_cast<float>(x))); }
    void _decode_float(float x) noexcept override { _s.append(fmt::format("float({})", luisa::bit_cast<uint>(x))); }
    void _decode_double(double x) noexcept override { _s.append(fmt::format("double({})", luisa::bit_cast<ulong>(x))); }
    void _vector_separator(const Type *type, uint index) noexcept override {
        auto n = type->dimension();
        if (index == 0u) {
            _s.append(fmt::format("{}{}(", type->element()->description(), n));
        } else if (index == n) {
            _s.append(")");
        } else {
            _s.append(", ");
    ...
    void _matrix_separator(const Type *type, uint index) noexcept override {
        auto n = type->dimension();
        if (index == 0u) {
            _s.append(fmt::format("{}{}x{}(", type->element()->description(), n, n));
        } else if (index == n) {
            _s.append(")");
        } else {
            _s.append(", ");
    ...
    void _struct_separator(const Type *type, uint index) noexcept override {
        auto n = type->members().size();
        if (index == 0u) {
            _s.append(fmt::format("struct<{}>", type->alignment())).append("{");
        } else if (index == n) {
            _s.append("}");
        } else {
            _s.append(", ");
