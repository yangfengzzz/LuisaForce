...
const Type *Type::buffer(const Type *elem) noexcept {
    ...
    return from(fmt::format("buffer<{}>", elem->description()));
}
...
const Type *Type::texture(const Type *elem, size_t dimension) noexcept {
    ...
    return from(fmt::format("texture<{},{}>", dimension, elem->description()));
}
...
bool Type::is_resource() const noexcept {
    return is_buffer() || is_texture() || is_bindless_array() || is_hash_grid();
}
